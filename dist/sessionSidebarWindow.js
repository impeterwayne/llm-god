...
function ensureCtxMenu() {
    if (ctxMenuEl && document.body.contains(ctxMenuEl))
        return ctxMenuEl;
    ...
    document.addEventListener("click", (e) => { if (!menu.contains(e.target))
        hideCtxMenu(); });
    document.addEventListener("contextmenu", (e) => { if (!menu.contains(e.target))
        hideCtxMenu(); });
    ctxMenuEl = menu;
    return menu;
}
function hideCtxMenu() { if (ctxMenuEl)
    ctxMenuEl.style.display = "none"; }
function showSessionContextMenu(x, y, s) {
    ...
    let mx = x, my = y;
    if (mx + r.width > vw)
        mx = Math.max(0, vw - r.width - 4);
    if (my + r.height > vh)
        my = Math.max(0, vh - r.height - 4);
