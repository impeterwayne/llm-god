...
function setCssSidebarWidth(px) {
    try {
...
function toggleSidebar(el) {
    ...
    if (goingToCollapsed) {
        ...
        try {
            localStorage.setItem("sessionsSidebar.collapsed", "true");
        }
        catch { }
    ...
    try {
        localStorage.setItem("sessionsSidebar.collapsed", "false");
    }
    catch { }
...
function programmaticCollapseSidebar(el) {
    if (el.classList.contains("collapsed"))
        return;
    el.classList.add("collapsed");
    document.body.classList.add("sidebar-collapsed");
...
function programmaticExpandSidebar(el) {
    if (!el.classList.contains("collapsed"))
        return;
    el.classList.remove("collapsed");
    document.body.classList.remove("sidebar-collapsed");
...
function renderSessions(listEl, sessions, activeId) {
    listEl.innerHTML = "";
    sessions.forEach((s) => {
...
export function initSessionSidebar() {
    ...
    if (!sidebar || !list || !newBtn || !toggleBtn)
        return;
    ...
    ipcRenderer.on("sessions:active-changed", (_event, payload) => {
...
function ensureSessionMenu() {
    if (sessionMenuEl && document.body.contains(sessionMenuEl))
...
function showSessionContextMenu(x, y, session) {
    ...
    let mx = x, my = y;
    if (mx + rect.width > vw)
        mx = Math.max(0, vw - rect.width - 4);
    if (my + rect.height > vh)
        my = Math.max(0, vh - rect.height - 4);
